#!/usr/bin/env bash
set -e

# ConnectifyVPN - Auto Installer (pakej lokal / git clone)
# Skrip ini akan:
# 1) (Jika boleh) auto set PERMISSION_URL berdasarkan repo Git yang anda clone
# 2) Salin pakej ke /opt/cvpn
# 3) Jalankan installer utama (/opt/cvpn/run)

if [[ $EUID -ne 0 ]]; then
  echo "Sila jalankan sebagai root: sudo bash $0"
  exit 1
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Jika skrip ini berada di root repo, folder cvpn biasanya wujud.
if [[ -d "${SCRIPT_DIR}/cvpn" ]]; then
  REPO_ROOT="${SCRIPT_DIR}"
  SRC_DIR="${SCRIPT_DIR}/cvpn"
else
  # Jika skrip ini berada dalam folder cvpn
  REPO_ROOT="${SCRIPT_DIR}"
  SRC_DIR="${SCRIPT_DIR}"
fi

# ─────────────────────────────────────────────────────────────
# Auto set PERMISSION_URL berdasarkan Git origin (jika ada)
# ─────────────────────────────────────────────────────────────
detect_raw_base() {
  local dir="$1"
  command -v git >/dev/null 2>&1 || return 1
  git -C "$dir" rev-parse --is-inside-work-tree >/dev/null 2>&1 || return 1

  local origin branch owner repo
  origin="$(git -C "$dir" remote get-url origin 2>/dev/null || true)"
  branch="$(git -C "$dir" rev-parse --abbrev-ref HEAD 2>/dev/null || true)"
  [[ -n "$branch" && "$branch" != "HEAD" ]] || branch="main"

  if [[ "$origin" =~ ^https?://github\.com/([^/]+)/([^/]+)(\.git)?$ ]]; then
    owner="${BASH_REMATCH[1]}"
    repo="${BASH_REMATCH[2]}"
  elif [[ "$origin" =~ ^git@github\.com:([^/]+)/([^/]+)(\.git)?$ ]]; then
    owner="${BASH_REMATCH[1]}"
    repo="${BASH_REMATCH[2]}"
  else
    return 1
  fi

  repo="${repo%.git}"
  echo "https://raw.githubusercontent.com/${owner}/${repo}/${branch}/"
}

CONF_DIR="/etc/cvpn"
CONF_FILE="${CONF_DIR}/permission.conf"
PERM_PATH="permission/ipuk/ip"

RAW_BASE="$(detect_raw_base "$REPO_ROOT" 2>/dev/null || true)"
if [[ -n "$RAW_BASE" ]]; then
  mkdir -p "$CONF_DIR"
  cat >"$CONF_FILE" <<EOF
# ConnectifyVPN - Konfigurasi kebenaran
PERMISSION_URL="${RAW_BASE%/}/${PERM_PATH}"
EOF
  chmod 600 "$CONF_FILE" >/dev/null 2>&1 || true
  echo "[*] PERMISSION_URL telah diset: ${RAW_BASE%/}/${PERM_PATH}"
else
  echo "[*] Tidak dapat detect Git origin (skip auto set PERMISSION_URL)."
  echo "    Jika perlu, anda boleh set manual dalam: ${CONF_FILE}"
fi

# ─────────────────────────────────────────────────────────────
# Salin pakej ke /opt dan jalankan installer utama
# ─────────────────────────────────────────────────────────────
INSTALL_DIR="/opt/cvpn"

echo "[*] Memasang ConnectifyVPN ke ${INSTALL_DIR} ..."
mkdir -p "${INSTALL_DIR}"
rm -rf "${INSTALL_DIR:?}/"* || true
cp -a "${SRC_DIR}/." "${INSTALL_DIR}/"

chmod +x "${INSTALL_DIR}/run"
if [[ -d "${INSTALL_DIR}/menu/menu" ]]; then
  chmod +x "${INSTALL_DIR}/menu/menu/"* || true
fi

# Keperluan asas
export DEBIAN_FRONTEND=noninteractive
apt-get update -y >/dev/null 2>&1 || true
apt-get install -y curl wget unzip jq git >/dev/null 2>&1 || true

echo "[*] Jalankan installer utama..."
exec bash "${INSTALL_DIR}/run"
